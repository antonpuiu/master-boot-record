...
void CREATE_PARTITION(FILE* f, char NUME_PARTITIE[8], int MAX_D, int MAX_F)
{
    ...
    NUMAR_PARTITII =
        fgetc(f); //Aflam din fisier numarul de partitii, acest numar fiind la inceputul fisierului.
    ADRESE = malloc(
        NUMAR_PARTITII *
        sizeof(
            char)); //Aloc dinamic memorie pentru fiecare adresa la care se gaseste cate o partitie.
    TOTAL_SPACE = malloc(NUMAR_PARTITII * sizeof(int));
    for (i = 0; i < NUMAR_PARTITII;
         i++) //In acest 'for' aflu adresele de inceput ale fiecarei partitii.
    {
        c = fgetc(f);
        while (c == 0) {
            c = fgetc(f);
    ...
    for (i = 0; i < NUMAR_PARTITII; i++) //Aflu total_space pentru fiecare partitie in parte.
    {
        ...
        fseek(f, 0, SEEK_SET);
        fseek(f, ADRESE[i] + 12,
              SEEK_CUR); //Mut cursorul in locul unde gasesc 'MAXD' in fisierul binar.
        MAXD = fgetc(f); //Retin 'MAXD'.
        fseek(f, 0, SEEK_SET);
        fseek(f, ADRESE[i] + 16,
              SEEK_CUR); //Mut cursorul in locul unde gasesc 'MAXF" in fisierul binar.
    ...
    while (1) {
        ...
        if (ftell(f) ==
            ADRESE[0]) //Are conditia de oprire atunci cand cursorul ajunge la prima adresa de partitie.
            break;
    ...
    if (ok == 1) {
        for (i = 0; i < NUMAR_PARTITII - 1; i++) {
            TOTAL_SPACE_NEED =
                20 + (MAX_F * 12) + (MAX_D * 12) +
                (MAX_D * MAX_F *
                 12); //Calculez spatiul total de care este nevoie pentru noua partitie.
            if (ADRESE[i + 1] - ADRESE[i] - TOTAL_SPACE[i] >= TOTAL_SPACE_NEED) //Daca am loc
            {
                fseek(f, POZITIE_PARTITIE_NOUA,
                      SEEK_SET); //incep introducerea partitiei in fisierul binar.
...
void FORMAT_PARTITION(FILE* f, char NUME_PARTITIE[8])
{
    ...
    for (i = 0; i < NUMAR_PARTITII; i++) {
        NUME_PARTITII[i] =
            malloc(8 * sizeof(char)); //Aloc dinamic memorie pentru numele fiecarei partitii.
    }
    for (i = 0; i < NUMAR_PARTITII; i++) {
        fseek(f, 0, SEEK_SET);
    ...
    for (i = 0; i < NUMAR_PARTITII; i++) {
        for (j = 0; j < 8;
             j++) //Prin aceste 2 for-uri verific daca partitia introdusa exista in fisierul binar.
        {
...
void VERIFICARE_MBR(FILE* f)
{
    ...
    for (i = 0; i < NUMAR_PARTITII; i++) //Verific daca fisierul binar are adresele scrise corect.
    {
        ...
        fseek(
            f, ADRESE[i] + 4,
            SEEK_CUR); //Setez cursorul la 4 pozitii dupa adresa de inceput a unei partitii. Acolo se regaseste numele partitiei.
        for (j = 0; j < 7; j++) {
            if ((c = fgetc(f)) !=
                NUME_PARTITII[j]) //Compar numele pe care il gasesc in fisier cu numele '/dev/sd'.

            {
                printf("INVALID\n");
                exit(
                    EXIT_FAILURE); //Inchei programul in cazul in care se gasesc diferente cu mesaj de eroare.
            }
        }
    }
    printf("SUCCES\n"); //In cazul in care nu exista diferente de nume, afisez 'SUCCES'.
    for (i = 0; i < NUMAR_PARTITII;
         i++) //Aflu used_space, total_space si numele partitiei pentru fiecare partitie in parte.
    {
        TOTAL_SPACE = 0;
        USED_SPACE = 0;
        fseek(f, 0, SEEK_SET);
        ...
        TOTAL_SPACE =
            MAXF * 12 + MAXD * 12 +
            MAXD * MAXF *
                12; //Calculez total_space. Aceasta variabila isi va pierde informatia dupa ce trec la urmatoarea partitie.
        ...
        if (MAXF != 0) //Aflu numarul fisierelor si numarul directoarelor.
        {
            fseek(f, 0, SEEK_SET);
            fseek(f, ADRESE[i] + 20,
                  SEEK_CUR); //Mut cursorul in locul in care incep informatiile despre directoare.
            for (j = 1; j <= MAXD; j++) {
                ...
                fseek(f, 0, SEEK_SET);
                fseek(
                    f, ADRESE[i] + 20 + (12 + (12 * MAXF)) * j,
                    SEEK_CUR); //Mut cursorul la urmatoarea aparitie a informatiei despre director.
            }
            fseek(f, 0, SEEK_SET);
            fseek(
                f, ADRESE[i] + 20 + 12,
                SEEK_CUR); //Mut cursorul in pozitia in care se regasesc informatii despre fisiere din directoare.
            for (j = 1; j <= MAXF; j++) {
                c = fgetc(f); //Variabila 'c' retine numarul de caractere al numelui fisierului.
                if (c !=
                    0) //Daca variabila 'c' este diferita de '0', inseamna ca exita fisierul, in consecinta cresc numarul de fisiere.
                    NR_FIS++;
                fseek(f, 0, SEEK_SET);
                fseek(
                    f, ADRESE[i] + 20 + (12 * j),
                    SEEK_CUR); //Mut cursorul la urmatoarea aparitie a informatiilor despre fisiere din directoare.
            }
        }
        USED_SPACE = NR_FIS * 12 + NR_DIR * 12 +
                     NR_DIR * NR_FIS * 12; //Aflu 'used_space' pentru partitia respectiva.
...
void DELETE_PARTITION(FILE* f, char* comanda)
{
    ...
    for (i = 0; i < NUMAR_PARTITII; i++) {
        NUME_PARTITII[i] =
            malloc(8 * sizeof(char)); //Aloc dinamic memorie pentru numele tuturor partitiilor.
    }
    for (i = 0; i < NUMAR_PARTITII; i++) {
        fseek(f, 0, SEEK_SET);
        fseek(f, ADRESE[i] + 4, SEEK_CUR);
...
int main(int argc, char* argv[])
{
    ...
    if (atoi(argv[1]) == argumente[0]) {
        mbr =
            fopen("mbr.bin",
                  "rb"); //Deschidem fisierul cu drepturi de citire si specificam tipul fisierului.
